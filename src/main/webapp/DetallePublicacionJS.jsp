...
<div id="contenedorAlertasJS" class="position-fixed top-0 end-0 p-3" style="z-index: 1100;"></div>
            ...
            <c:if test="${not empty esAdminPub}">
                ...
                <c:when test="${haCompradoORentado}">
                  <div class="card shadow-sm p-4 mb-4 rounded-4 border-0">
                    <form id="formCrearResena">
                      ...
                      <button type="submit" id="btnEnviarResena" class="btn btn-action-lf shadow-sm px-4">
                        Enviar reseña
                      </button>
                    </form>
                  </div>
              ...
              <c:choose>
                <c:when test="${empty resenas}">
                  <div id="alertaSinResenas" class="alert alert-light border rounded-4 text-muted mb-0 mt-3">
                    <i class="bi bi-info-circle me-2"></i> No hay reseñas aún. ¡Sé el primero en opinar sobre este libro!
                  </div>
                  <div id="contenedorResenas" class="pe-2 mt-3"></div>
                </c:when>

                <c:otherwise>
                  <div id="contenedorResenas" class="pe-2 mt-3">
                    <c:forEach var="r" items="${resenas}">
  ...
  <div class="card p-3 mb-3 shadow-sm rounded-4 border-0 bg-light">
    <div class="d-flex justify-content-between align-items-center mb-1">
      <div class="d-flex align-items-center gap-3">
        ...
        <strong class="text-dark nombre-usuario"></strong>
      </div>
      <div class="estrellas-contenedor"></div>
    </div>
    <p class="mb-0 text-secondary texto-comentario"></p>
  </div>
</template>

<template id="templateAlerta">
  <div class="libri-toast mb-3 toast-alerta" style="animation: slideDown 0.4s ease-out forwards;">
    <i class="bi fs-5 icono-alerta"></i>
    <span class="texto-alerta"></span>
  </div>
</template>
...
<script src="${pageContext.request.contextPath}/assets/js/ResenaJS.js"></script>
</body>
</html>